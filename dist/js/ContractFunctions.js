...
async function fetchAccountDetails(userAddress) {
    try {
        ...
        safeSetTextContent('totalWithdrawals', web3.utils.fromWei(details.totalWithdrawals.toString(), 'ether'));
...
async function createUserPackagesTable(userAddress, currentPage = 1, recordsPerPage = 20) {
    ...
    try {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            const withdrawalClass = pkg.isWithdrawn ? 'text-success' : 'text-pending'; // Consider "text-warning" for pending
            
            // Modify the status text and class based on endDate and withdrawal status
            let statusText, statusClass;
            if (new Date() > endDateValue) { // If the current date is past the end date
                statusText = 'Finished';
                statusClass = 'text-pending'; // Consider changing the class based on your color scheme
            } else {
                statusText = 'Running';
...
function openPopup(packageDetails) {
    ...
    popupContent.innerHTML = `
        ...
        <strong>Daily Profit:</strong> $${dailyReturn} (Per day)<br>
